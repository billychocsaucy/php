...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    ************************************************************************ */
    ...
    #$userchoice checking
    if (empty($name)){ #Check if user input is empty
        $errmsg = 1;
        $namerr = "<span class='error'> Your password is missing!! </span>";
    } else {
        if (strlen($name) < 10) { #Check if user input is valid
            $errmsg = 1;
            $namerr = "<span class='error'> Not enough characters in passwrord!! </span><br>";
        } else {
    ...
    ************************************************************************ */
    ...
    if ($errmsg != 0) {
        ...
        echo "<p class='success'>Great job! There are no errors.</p>";

    #URL Field
        echo "<h3>Email Field</h3>";
        echo $_POST['email'];
    ...
    #Radio Field
        echo "<h3>Color Field</h3>";
        echo $_POST['color'];

    #Select Field
        echo "<h3>Favorite State Field</h3>";
        echo $_POST['favstate'];

    #Text area Field
        echo "<h3>Hobbies Field</h3>";
        echo $_POST['hobbies'];
        echo strlen($_POST['hobbies']);
        echo "$hobbies";
        echo strlen($hobbies);
...
if ($showform == 1)
{
    ...
    <!-- Username code -->
        ...
        ?>
        <br><label for="name">What is your username? (Must be at least 10 Characters) </label>
        <input type="text" name="name" id="name" placeholder="Username" value="<?php if(isset($name)){ echo htmlspecialchars($name);}?>"><br><br>
    ...
    <!-- Favorite State code -->
        ...
        <label for="favstate">Which state do you like more: </label><select name="favstate" id="favstate">
            <option value=""<?php if (isset($favstate) && $favstate == "") { echo " selected ";}?>
            >CHOOSE A STATE</option>
            <option value="TX"<?php if (isset($favstate) && $favstate == "TX") { echo " selected ";}?>
            >Texas</option>
            <option value="FL"<?php if (isset($favstate) && $favstate == "FL") { echo " selected ";}?>
            >Florida</option>
        </select><br><br>
